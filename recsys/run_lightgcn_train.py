...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a local LightGCN baseline recommender.")
...
def main() -> None:
    ...
    recommendations = build_lightgcn_recommendations(
        dataset=dataset,
        training_result=result,
        top_k=args.top_k,
    )
    payloads = build_lightgcn_firestore_payloads(
        dataset=dataset,
        training_result=result,
        top_k=args.top_k,
    )
    ...
    _write_json(output_dir / "lightgcn_prep_summary.json", prep.summary())
    _write_json(output_dir / "lightgcn_training_summary.json", result.summary())
    _write_json(output_dir / "lightgcn_losses.json", result.losses)
    _write_json(
        output_dir / "lightgcn_embeddings.json",
        {
            "students": result.student_embeddings,
            "groups": result.group_embeddings,
        },
    )
    _write_json(output_dir / "lightgcn_recommendations.json", recommendations)
    _write_json(output_dir / "lightgcn_firestore_payloads.json", payloads)

    print(f"Trained local LightGCN baseline in {output_dir}")
    print(
        f"epochs={len(result.losses)} "
        f"final_loss={result.final_loss:.6f} "
        f"student_embeddings={len(result.student_embeddings)} "
        f"group_embeddings={len(result.group_embeddings)}"
    )
